                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                var districtadd = '{{ get_districts() }}';
                                var decodedDistrict = districtadd.replace(/&quot;/g, '"');
                                var districtArray = JSON.parse(decodedDistrict);
                                var user = response?.useradd;
                                var addressadd = response?.address ?? 0;
                                var district = addressadd.district_id ?? 0;
                                //var districtId = addressadd.district_id ?? 0;
                                let options = '';
                                districtArray.forEach(element => {
                                    options +=
                                        `<option value="${element?.id}" ${element?.id == district ? 'selected':''}>${element.district_name_bn}</option>`;
                                });
                                //console.log(options);
                                var districtId = addressadd.district_id;
                                ...
                                upazillasData.forEach(element => {
                                    UpazillaOptions +=
                                        `<option value="${element.id}" ${element?.id == upazillaId ? 'selected':''}>${element.name_bn}</option>`;
                                });
                                console.log(upazillasData);
                                if (user)
