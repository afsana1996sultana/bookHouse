...
<section class="content-main">
    <div class="content-header">
        <h2 class="content-title">Subject List <span class="badge rounded-pill alert-success"> {{ count($categories) }} </span></h2>
        <div>
            @if(Auth::guard('admin')->user()->role !== '5' || in_array('6', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                <a href="{{ route('category.create') }}" class="btn btn-primary"><i class="material-icons md-plus"></i> Add Subject</a>
            @endif
        </div>
    </div>
    </div>
